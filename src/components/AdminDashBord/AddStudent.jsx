...
const AddStudent = () => {
  ...
  const fetchCourses = () => {
    ...
    axios
      .get("https://campus-9pqa.onrender.com/course/viewall", {
        headers: { token: sessionStorage.getItem("token") },
      })
      .then((response) => {
        if (response.data.status === "error") {
  ...
  const submitHandler = () => {
    axios
      .post("https://campus-9pqa.onrender.com/student/addStudent", input, {
        headers: { token: sessionStorage.getItem("token") },
      })
      .then((response) => {
        if (response.data.status === "error") {
  ...
  return (
    <div>
      ...
      <div className="container-fluid">
        <h1 style={{ fontFamily: "fantasy" }}>ADD STUDENT</h1>
        <hr />
        <div className="row">
